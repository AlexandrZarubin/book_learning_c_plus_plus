...
int main()
{
    ...
    int score = 0; // начальное количество очков
    const int hintPenalty = 2; // штраф за использование подсказки

    // Приветствие и правила игры
    cout << "\t\t\tДобро пожаловать в игру Word Jumble!\n\n";
    cout << "Разгадайте слово, переставив буквы.\n";
    cout << "Введите 'hint' для (стоимость " << hintPenalty << " очков).\n";
    ...
    while ((guess != theWord) && (guess != "quit"))
    {
        if (guess == "hint") // если игрок просит подсказку
        {
            ...
            score -= hintPenalty; // уменьшение очков за использование подсказки
            cout << "\nВы потеряли " << hintPenalty << " очка за использование подсказки.";
        }
    ...
    if (guess == theWord)
    {
        score += length; // начисление очков за отгаданное слово (количество букв в слове)
        cout << "\nВерно! Вы угадали!\n";
        cout << "Вы заработали " << length << " очков за угаданное слово.\n";
    }
